...
class SessionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('kk')) {
            ...
            $file->move(public_path('fileCatatan'), $filenamekk);
        }
        if ($request->hasFile('ktp')) {
            $file = $request->file('ktp');
            $filenamektp = $file->getClientOriginalName();
            $file->move(public_path('fileCatatan'), $filenamektp);
        }
        if ($request->hasFile('pas_foto')) {
            $file = $request->file('pas_foto');
            $filenamepass = $file->getClientOriginalName();
            $file->move(public_path('fileCatatan'), $filenamepass);
        }
        ...
        catatan::create([
            ...
            'kk' => $filenamekk,
            'ktp' => $filenamektp,
            'pas_foto' => $filenamepass,
        ]);

        return redirect()->route('dashboard')->with('Success', 'Pengajuan SIM berhasil diajukan!');
        Activity::create([
            'user_id' => Auth::id(),
